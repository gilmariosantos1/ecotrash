...
const PainelMunicipioDetalhes: React.FC = () => {
  ...
  const badgeStatus = (status: string): string => {
    const map: Record<string, string> = {
      'Agendado': 'badge-processado', 'Coletado': 'badge-coletado',
  ...
  return (
    <IonPage>
      <EcoHeader onBack={() => history.push('/municipio/lista', sessao)} />
      <IonContent>
        <div className="page-wrapper" style={{ padding: '16px' }}>
          <div style={{ background: 'rgba(0,0,0,0.75)', border: '1px solid rgba(92,184,92,0.25)', borderRadius: 16, padding: '24px 20px', backdropFilter: 'blur(8px)' }}>
            <h2 style={{ color: '#5cb85c', fontWeight: 700, marginBottom: 20 }}>Detalhe do Requerimento</h2>

            {carregando ? (
              <div style={{ textAlign: 'center', padding: 40 }}><IonSpinner color="primary" /></div>
            ) : !pedido ? (
              <p style={{ color: 'rgba(255,255,255,0.7)', textAlign: 'center' }}>Requerimento não encontrado.</p>
            ) : (
              <>
                {/* Status + Data */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                  <div><p className="req-item-label">Data do pedido</p><p className="req-item-value">{pedido.dataRequisicao}</p></div>
                ...
                {[
                  ...
                  <div key={label} style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: 10, marginBottom: 10 }}>
                ...
                {/* Endereço */}
                <div style={{ paddingBottom: 10, marginBottom: 16, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                ...
                {pedido.status === 'Em análise' && (
                  <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
                    <IonButton expand="block" style={{ flex: 1, '--background': '#5cb85c', '--color': '#fff', textTransform: 'none' }} onClick={aceitar}>
                      <IonIcon icon={checkmarkCircleOutline} slot="start" />Aceitar
                    </IonButton>
                    <IonButton expand="block" style={{ flex: 1, '--background': '#eb445a', '--color': '#fff', textTransform: 'none' }} onClick={recusar}>
                      <IonIcon icon={closeCircleOutline} slot="start" />Recusar
                    </IonButton>
                  </div>
                )}
