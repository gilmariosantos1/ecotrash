...
export default function CadastroMunicipio() {
  ...
  const carregarEstados = async () => {
    ...
    setErro('');
    try {
      const data = await LocalidadeController.carregarEstados();
  ...
  const handleEstadoChange = async (e) => {
    ...
    if (uf) {
      ...
      try {
        const data = await LocalidadeController.carregarMunicipios(uf);
  ...
  return (
    <>
      ...
      <div className="form-container">
        ...
        {erro && (
          <div style={{ padding: '10px', marginBottom: '15px', backgroundColor: '#fee', color: '#c00', borderRadius: '6px', border: '1px solid #fcc' }}>
            <div>{erro}</div>
            <button type="button" onClick={carregarEstados} style={{ marginTop: '8px' }}>Tentar novamente</button>
          </div>
        )}
